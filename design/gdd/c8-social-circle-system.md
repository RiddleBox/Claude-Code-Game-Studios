...
3. **活跃名单轮转 (Active List Rotation)**：
   - **活跃定义**：只有处于 Active List 的 NPC 会被注入 F6 的上下文，角色在对话中会表现出对这些人的深刻记忆。
   - **激活机制**：C4 剧情强制激活，或在归来时从池中按权重随机挑选。
   - **退出机制**：采用”近 30 天提及次数 + 亲密度”综合排序。若槽位满，优先移除 `mention_count` 最少且 `closeness` 低于阈值的 NPC。
   - **触发时机**：**仅在 C2「角色归来」事件时触发**，不在每日登录时触发。（原因：归来是叙事节奏的自然切分点，频繁轮转会破坏叙事连贯性。）
